...
class HomeScreen(MDScreen, ThemableBehavior):
    ...
    active, first_use, ignore_release, add_picture, change_home = True, True, False, False, False
    list, source_list, prev_list, delete_dict = [], [], [], {}
    ...
    def on_enter(self, *args):
        if self.first_use and FIRST_TIME:
            ...
            self.update_home() if self.add_picture else app.InfoScreen.check_object_changed()
            # TODO: Update the dictionary in self.list when the textfield is changed
            #  place is updated. Also add a Floating Camera button to update existing pic.
    ...
    def update_home(self):
        ...
        self.grid.add_widget(
            MyImage(source='%s.png' % app.InfoScreen.ids.object.text,
                    ...
                    index=self.index))
        self.source_list.append(app.InfoScreen.ids.object.text)
        self.index += 1
