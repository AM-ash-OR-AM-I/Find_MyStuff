...
class CustomDialog(AKAlertDialog):
    header_text_type = 'text'
    header_height_portrait = 0

    def on_dismiss(self):
        try:
            app.ObjectInfo.stop_anim()
...
class ObjectInfo(MDScreen):
    ...
    win_listen = False
    camera_button_exists = BooleanProperty(True)
    dict = {}
    first_call = True
    object, place = '', ''
    ...
    def __init__(self, **kwargs):
        ...
        self.content_cls = VoiceRecognizer()
        self.dialog = CustomDialog(size_portrait=[dp(268), dp(268)],
                                   pos_hint={'center_y': .55})
        self.dialog.size_hint = (dp(268) / Window.width, dp(268) / Window.height)
        self.dialog.content_cls = self.content_cls
        self.dialog._anim_duration = .1
    ...
    def open_dialog(self):
        ...
        self.ids.place.text = ''
        self.camera_button_exists = True
    ...
    def check_object_changed(self, *args):
        source_lst = app.HomeScreen.source_list
        new_object = self.ids.object.text
        lst = app.HomeScreen.list
        print(f'{source_lst = }')
        if new_object!= self.previous_object:
            if self.previous_object in source_lst:
                os.rename(self.previous_object + '.png', new_object+ '.png')
                index = lst.index(self.previous_object)
                lst.remove(self.previous_object)
                lst.insert(new_object, index)
                print(f'{lst = }]\n{app.HomeScreen.list = }')

        elif self.ids.place.text != self.previous_place:
            if self.previous_place in lst.values():
                index = lst.index(self.previous_place)
                # lst[index]
                print(f'{lst = }]\n{app.HomeScreen.list = }')
            print('Place Changed')
    ...
    def on_camera_button_exists(self, inst, exists):
        layout = self.ids.relative
        if exists:
            for widget in self.ids.relative.children:
                if isinstance(widget, MDRoundFlatIconButton):
                    return
            layout.add_widget(MDRoundFlatIconButton(text='Take Picture', icon='camera-plus',
                                                    pos_hint={'center_y': .5, 'center_x': .5}, on_release=self.save))
    ...
    def image_clicked(self, image):
        app.change_screen('ObjectInfo')
        self.camera_button_exists = False
    ...
    def add_info_object(self, *args):
        ...
        def add_pic(*args):
            ...
            self.ids.circle_bounce.active = False
        ...
        self.ids.circle_bounce.active = True
        if ANDROID:
...
class VoiceRecognizer(MDBoxLayout):
    pass
