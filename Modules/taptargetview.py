...
class MDTapTargetView(ThemableBehavior, EventDispatcher):
    ...
    outer_circle_alpha = NumericProperty(.5)
    """
    ...
    cancelable = BooleanProperty(True)
    """
    ...
    def _fix_elev(self):
        with self.widget.canvas.before:
            ...
            Rectangle(
                ...
                size=self.widget.soft_shadow_size,
                pos=self.widget.soft_shadow_pos,
            )
            Color(a=self.widget._hard_shadow_a)
            Rectangle(
                texture=self.widget._hard_shadow_texture,
                size=self.widget.hard_shadow_size,
                pos=self.widget.hard_shadow_pos,
            )
            Color(a=1)
